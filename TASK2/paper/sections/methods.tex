...
Mortality prediction was evaluated using six supervised learning models: logistic regression (LR), support vector machine (SVM), k-nearest neighbors (KNN), random forest (RF), extreme gradient boosting (XGBoost), and multilayer perceptron (MLP). Two experiments were conducted. Experiment 1 served as the primary model benchmark and compared all six models on the scaled dataset. This setting was chosen to provide a consistent comparison across model classes while avoiding an unfair disadvantage for scale-sensitive methods. Experiment 2 was a preprocessing sensitivity analysis in which the tree-based models RF and XGBoost were evaluated on raw, winsorized, and scaled versions of the dataset. Because winsorization was introduced only for robustness comparison, the scaled benchmark remained the main reference setting for interpretation.

Logistic regression was included as a conventional and interpretable baseline for binary classification. For patient feature vector $\mathbf{x}$, logistic regression estimates the probability of death as
\begin{equation}
P(y=1 \mid \mathbf{x}) = \sigma(\beta_0 + \boldsymbol{\beta}^{\top}\mathbf{x}) = \frac{1}{1 + e^{-(\beta_0 + \boldsymbol{\beta}^{\top}\mathbf{x})}}.
\end{equation}

The SVM model was included as a margin-based classifier capable of learning nonlinear decision boundaries through kernel mapping. KNN was included as a distance-based nonparametric baseline in which a patient's class is determined from the labels of nearby samples in feature space. Both methods are sensitive to feature scale, and therefore they were evaluated only in the scaled-data benchmark.

For SVM, the decision function can be written as
\begin{equation}
f(\mathbf{x}) = \operatorname{sign}\left(\sum_{i=1}^{n}\alpha_i y_i K(\mathbf{x}_i,\mathbf{x}) + b\right),
\end{equation}
where $\alpha_i$ are learned coefficients, $y_i$ are class labels, $K(\mathbf{x}_i,\mathbf{x})$ is the kernel function, and $b$ is the bias term. This formulation allows the classifier to separate classes by maximizing the margin in a transformed feature space.

For KNN, the predicted class for a sample $\mathbf{x}$ is determined by majority vote among its $k$ nearest neighbors:
\begin{equation}
\hat{y}(\mathbf{x}) = \operatorname{mode}\{y_i : \mathbf{x}_i \in \mathcal{N}_k(\mathbf{x})\},
\end{equation}
where $\mathcal{N}_k(\mathbf{x})$ denotes the set of the $k$ closest training samples to $\mathbf{x}$ under the chosen distance metric. This method provides a simple reference model based on local similarity structure in the feature space.
...
Model performance was evaluated using accuracy, precision, recall, F1-score, and the area under the receiver operating characteristic curve (AUC). The F1-score is defined as
\begin{equation}
